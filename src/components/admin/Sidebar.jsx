  ...
  return (
    ...
    >
      ...
      >
        {open && (
          <div className="ps-2">
            ...
            <small
              ...
              style={{ fontSize: 10, display: "block", marginTop: "-4px" }}
            >
              ADMINISTRATION
            </small>
          </div>
        ...
        <button
          className="btn btn-sm btn-light ms-auto rounded-circle border-0 shadow-sm"
          onClick={() => setOpen((v) => !v)}
      ...
      {/* Nav */}
      <nav className="flex-grow-1 px-3 mt-2" style={{ overflowY: "auto" }}>
        {MENU.map((m) => (
          <button
            key={m.key}
          ...
          >
            <span
              style={{
                fontSize: 18,
                filter: active === m.key ? "none" : "grayscale(1)",
              }}
            >
              {m.icon}
            </span>
